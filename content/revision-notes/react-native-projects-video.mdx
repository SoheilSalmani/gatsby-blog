...
If you're on a Mac and developing for iOS, you need to install the pods (via
Cocoapods) to complete the linking.

```sh
npx pod-install ios
```

To finalize installation of `react-native-gesture-handler`, add the following at
the **top** (make sure it's at the top and there's nothing else before it) of
your entry file, such as `index.js` or `App.js`.

Now, we need to wrap the whole app in `NavigationContainer`. Usually you'd do
...
The goal of the application is to store, create, and modify notes on a mobile device.

Planning:

* Step 1: Each page has a common layout
* Step 2: We want access to our same data list throughout our application to modify it
* Step 3: We want to be able to navigate through multiple screens

Key Stories:

* As a user, I would like the style of the application to be consistent on each page
